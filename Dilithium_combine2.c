...
int32_t PQCLEAN_DILITHIUM2_CLEAN_power2round(int32_t *a0, int32_t a)  { // t = t1*s^d + t0 로 t1, t0를 생성하는 코드
    int32_t a1;

    a1 = (a + (1 << (D - 1)) - 1) >> D;     // t1은 결국 t를 2^d로 나누었을 때의 몫이라고 볼 수 있음. 근데, 여기에서 
    *a0 = a - (a1 << D);
    return a1;
}
...
static unsigned int rej_uniform(int32_t *a, unsigned int len, const uint8_t *buf, unsigned int buflen) {
    ...
    ctr = pos = 0;
    while (ctr < len && pos + 3 <= buflen) {    // 3byte    즉, 24bit씩 가져와서 
...
int PQCLEAN_DILITHIUM2_CLEAN_crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
    ...
    PQCLEAN_DILITHIUM2_CLEAN_polyveck_uniform_eta(&s2, rhoprime, DILITHIUM_L);
